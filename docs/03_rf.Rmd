...
```{r rfTreeBoot1, fig.cap="Classification tree obtained with the default values of `rpart()` on the bootstrap sample `spamBoot1`; `spam` data.", fig.width=8.5, fig.height=6}
set.seed(368910)
...
```{r rfTreeBoot2, fig.cap="Classification tree obtained with the default values of `rpart()` on the bootstrap sample `spamBoot1`; `spam` data.", fig.width=8.5, fig.height=6}
set.seed(368915)
...
```{r rfPlotRFDefEcho, fig.cap= "Evolution of the global OOB error and for each class, according to the number of trees, `spam` data."}
plot(RFDef)
```
...
```{r rfOzoneRFDefRes, fig.cap="OOB error as a function of the number of trees, `Ozone` data."}
OzRFDef
plot(OzRFDef)
```

```{r rfOzoneMtryPlot, fig.cap="OOB error as a function of `mtry` parameter value, `Ozone` data."}
...
```{r rfVac18RFpsur3, fig.cap="OOB error evolution according to the number of trees in the forest (OOB errors per class have been hidden to facilitate the reading of the graph), `Vac18` data."}
...
```{r rfPM10partialNO, fig.cap="Marginal effect of NO variable on PM10 concentration, associated with a random forest trained using station JUS data."}
