...
```{r rfTreeBoot1, fig.cap="Arbre de classification obtenu avec les valeurs par défaut de `rpart()` sur l'échantillon bootstrap `spamBoot1`", fig.width=8.5, fig.height=6}
set.seed(368910)
...
```{r rfTreeBoot2, fig.cap="Arbre de classification obtenu avec les valeurs par défaut de `rpart()` sur l'échantillon bootstrap `spamBoot2`.", fig.width=8.5, fig.height=6}
set.seed(368915)
...
```{r rfOzoneRFDefRes, fig.cap="Evolution de l'erreur OOB en fonction du nombre d'abres, données `Ozone`."}
OzRFDef
plot(OzRFDef)

```{r rfOzoneMtryPlot, fig.cap="Evolution de l'erreur OOB en fonction de la valeur du paramètre `mtry`, données `Ozone`."}
...
```{r rfPM10partialNO, fig.cap="Effet marginal de la variable NO, associé à une forêt aléatoire entraînée en la station JUS"}
